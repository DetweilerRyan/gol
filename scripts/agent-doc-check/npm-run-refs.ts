 ...
 * read as a real reference, so a bare kebab-case placeholder (e.g. "npm run
 * some-script", with no brackets) reads exactly like a real one and check1
 * will flag it as unresolved. Write a hypothetical example as `npm run
 * <script-name>` instead -- angle brackets are the escape hatch, since `<`
 * falls outside this regex's character class.
 */
...
// whatever punctuation/backtick markdown wraps the reference in. Caught
// during this program's own development: an early draft of one of these
// doc comments did exactly that.
