 ...
 * used to exist but were retired. Hand-kept rather than discovered off disk
 * at runtime, and deliberately small: `git log --all --diff-filter=D
 ...
 * `refactorer` became `cleaner`. The tradeoff is real -- a role retired
 * after this list was last updated won't be caught until someone adds it
 * here -- but role retirements are rare, deliberate, user-directed events
 * (see workflow.md), which is exactly the kind of change that should also
 * touch this list.
 */
...
// was for: a stale reference to a role that used to exist.
