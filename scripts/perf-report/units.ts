...
//
// The list below was built by inspecting a real Performance.getMetrics()
// response captured against this app (see the raw sample referenced above):
// every "*Duration"-named key is CDP-seconds, and so are
// ThreadTime/ProcessTime -- CPU-time counters with no "Duration" in their
// name but the same seconds magnitude and the same base::TimeDelta-backed
// accounting on Chromium's side (confirmed empirically: ThreadTime and
// TaskDuration land within a few percent of each other on every rep).
