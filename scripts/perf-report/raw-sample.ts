...
function isFiniteNumber(value: unknown): value is number {
  ...
  // false for every non-number. Measured on
  // scripts-mutation-survivors-untriaged's tree -- forcing that operand true
  // left all 654 of `npm run test:scripts` green. Kept because it is what
  // makes the `value is number` predicate readable at a glance, and because
  // the `&&` -> `||` mutant at the same site was a real kill on that same run
  // (5 red), so the operator here is load-bearing even though the operand is
  // not.
