...
- A plain-English end-to-end QA outline per feature — the user-visible workflows, inputs, and observable states the `qa` role will later turn into a `features/*.e2e.spec.ts` Playwright suite. This describes what to verify through the actual UI, not implementation internals.
...
5. **Sketch the QA outline** — for anything in the feature that's genuinely DOM/interaction behavior rather than testable pure logic (matching this repo's existing split between the Gherkin/`.steps.test.ts` layer and the `features/*.e2e.spec.ts` layer — see `CLAUDE.md`'s Testing structure section), write a short plain-English outline of the user-visible workflows, inputs, and observable states qa should verify end-to-end through the real UI.
