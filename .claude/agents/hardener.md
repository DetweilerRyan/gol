...
- The complete final verification sequence for a feature, run in order, fixing whatever each stage finds before moving to the next:
  ...
  8. `npm run ast-grep:rules` — a sub-second gate confirming the structural rules are actually live: fixture pairing, `id`/filename agreement, fixture `id` naming a real rule, no duplicate ids, a present `severity` key, every fixture carrying at least one `invalid:` case, and every `files:` glob resolving. `architect` owns `rules/*.yml` and runs this too, but it belongs here as well because the check most likely to fire is invalidated by changes to **`src/`, not to `rules/`** — moving or renaming a file can silently disarm a rule scoped to it, during a slice that never edits a rule at all. If it fails, report it rather than editing `rules/`: those are `architect`'s. Also run `npm run ast-grep` and read its output; findings are warning-severity and don't move the exit code, so a zero exit is not evidence of anything.
