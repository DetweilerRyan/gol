...
- **Each hook in `src/hooks/` owns exactly one piece of state or one browser API, and delegates the actual rules to a framework-free module.** That split is what keeps the rules testable without a jsdom environment. Put new logic in the pure module and let the hook stay a thin adapter.
  ...
  **What each hook does is its own hover, not this article's business.** CLAUDE.md's compact module map names them; the JSDoc above each declaration says what it owns and why. This article carries only what **no single hover can hold**. Three shapes qualify: a contract between two modules, a dependency a distant module rests on, and an obligation whose audience never opens the owning module. Five such constraints follow.

  - **`useMatchMedia.ts` — a test that reaches it must stub it.** jsdom does not implement `matchMedia` at all. Any test mounting a tree that reaches this hook must call `src/test-support/domStubs.ts`'s `stubMatchMedia`. That holds whether the test reaches it directly or through `useReducedMotion.ts` or `useSystemAppearance.ts`. **It carries no defensive fallback on purpose**: an unstubbed test throwing loudly beats shipping a branch no gate can exercise. This binds whoever writes a component test whose tree touches appearance or reduced motion, and that person never opens `useMatchMedia.ts`.
  - **`useAppearance.ts` — call it exactly once, from `App.tsx`, and nothing enforces that.** A second call site would hold a second `useState` the two could desync against. The hover warns whoever types the call; this entry binds whoever is deciding where appearance state lives, who may never type it.
  - **`useCamera.ts` and `useZoomGlide.ts` — every camera write cancels an in-flight glide first.** `useCamera`'s private `commit()` is the single funnel. It exists because reset is only one of five external writers: wheel zoom, drag-pan, scrollbar drag and the arrow-key reveal-pan are the others. One cancel site is what stops the fifth being forgotten. **The glide's own frames call `setCamera` directly and deliberately bypass `commit()`.** Neither hover states this pair; it is a contract between two hooks.
  - **`useZoomGlide.ts` and `useRafCoalescedPan.ts` — the two animation-frame owners differ deliberately at unmount.** An unfinished glide is **cancelled, never flushed**, where a coalesced pan flushes. A coalesced pan owes its caller an already-accumulated delta. A half-arrived-at cellSize is owed to nobody. The contract _is_ the contrast, so it cannot live in either hover alone.
  - **`useCellTiles.ts`'s reference-stable range is load-bearing three modules away.** The hook's own hover states the guarantee — an unchanged camera hands back the same `range` object. What no hover can state is who rests on it. The no-re-render-inside-the-hysteresis property at the bottom of this section holds only because `liveCellsInRange`'s range argument is unchanged **by reference**. Break the stability and that property goes quietly.

  A sixth was drafted and then cut — `useGridPointerGestures.ts`'s rect-relative-pixels boundary. Its own hover states the rule _and_ the caller's half of it. The entry was a second copy of a hover, not something no hover could hold.
