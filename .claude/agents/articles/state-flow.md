...
- **Each hook in `src/hooks/` owns exactly one piece of state or one browser API, and delegates the actual rules to a framework-free module.** That split is what keeps the rules testable without a jsdom environment. Put new logic in the pure module and let the hook stay a thin adapter.
  ...
  **What each hook does is its own hover, not this article's business.** CLAUDE.md's compact module map names them; the JSDoc above each declaration says what it owns and why. This article carries only what **no single hover can hold** — a constraint whose audience never opens the module that owns it. Six such constraints follow.
  ...
  - **`useAppearance.ts` — call it exactly once, from `App.tsx`.** A second call site would hold a second `useState` the two could desync against. Nothing enforces it.
  ...
  - **`useCellTiles.ts` — the returned range is reference-stable by `nextTileRange`'s own contract.** A pan inside the eviction hysteresis rests on that, three modules away. The guarantee is stated at the bottom of this section rather than in the hook.
  - **`useGridPointerGestures.ts` — it reports rect-relative pixels and never touches a `Camera`.** That says who owns the world-coordinate conversion, which is a boundary between this hook and `camera.ts` rather than a fact about either.
