...
- `scripts/perf-report/` — turns the raw JSON files `perf/` writes under `reports/perf/raw/` into `reports/perf/latest.md`, `latest.json` and `history.jsonl`. Report-only, no thresholds.
