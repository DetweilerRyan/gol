...
- **Never commit to `main`.** `main` is written to only by the merge protocol in `.claude/references/merge-protocol.md`, which the orchestrating session drives. If you find yourself on `main`, or in a directory that isn't your slice's worktree, stop and report it rather than committing.
