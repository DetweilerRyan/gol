...
- `hardener` **adds** `npm run test:mutation:scripts`, `npm run crap4ts:scripts` and `npm run dry4ts:scripts` whenever the slice touched `scripts/`. **They add to the `src/`-scoped runs rather than replacing them**: a slice can touch both trees, and the two mutation runs mutate disjoint file sets — `stryker.config.json`'s `mutate` is `src/**`, `stryker.scripts.config.json`'s is `scripts/**`. Skipping one because the other ran leaves that tree unmeasured. **Mutation thresholds are not the same on the two sides.** Read each from its own config, `stryker.config.json` and `stryker.scripts.config.json`, rather than from a figure restated here. `crap4ts:scripts` scores against `coverage-scripts/coverage-final.json`, so run `npm run test:coverage:scripts` first, the way you would `npm run test:coverage` before `npm run crap4ts`.
...
- Every role whose slice reaches what the test suites read must run the relevant tests before handoff and fix any failures. (SPECIFY runs before any implementation exists, so there is nothing yet to verify; `product` in VERIFY mode runs everything; the process roles' corpus is outside every suite, so their own files name their gates instead.) In this repo that is `npm run test:unit` (fast path — unit tests + Gherkin acceptance tests, excludes property tests) unless a role's own file specifies the full `npm test` instead.
