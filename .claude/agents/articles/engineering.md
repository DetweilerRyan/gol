...
- **Black-box e2e spec** (`e2e/*.e2e.spec.ts`, `npm run test:e2e`) — when the test boots the app and asserts user-visible behavior through the UI. This layer belongs to `qa` alone, and only `qa` writes it. Needing a real browser is not what makes a test an e2e spec; being about a user-facing feature rather than an implementation's contract is.
...
- Whole-suite commands are safe to run concurrently **across worktrees**: each slice's worktree has its own dev-server port (`dev-port.ts`), its own `node_modules`, and its own `coverage/`, `reports/`, `test-results/`, `.stryker-tmp*`, and `dist/`.
