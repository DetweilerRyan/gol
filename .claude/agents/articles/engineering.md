...
- **`cleaner` and `architect` may update the docs as part of a split they perform.** Both charters already include behavior-preserving splits/relocations. When one of them does that, **update both surfaces**. `CLAUDE.md`'s compact module map carries names and layer only; `architecture.md` or `state-flow.md` in this directory carries the contracts that span modules. Per-module detail belongs in the module's own JSDoc hover. The map is a routing index and the article is the account, so updating only one leaves the other lying. Fix any file-list mention elsewhere in `CLAUDE.md` or `.claude/agents/**` that the split just made stale. All of this is normal, expected work and needs no separate authorization.
