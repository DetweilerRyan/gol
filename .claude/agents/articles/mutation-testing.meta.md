...
**`adr/**`** — the decision-record tier, added by `wire-in-the-adr-tier` on the user's 2026-09-16 ruling after that slice paid a full stage-5 run for a documentation-only diff. The `sharedExclude` entry predates this row — `vale-styles-is-reachable-by-vitests-default-include` added it in the reviewed-precondition order this tier requires — and was verified present 2026-09-16 by parsing the array; C1 re-verifies it against every project's own exclude on every run. The directory holds `.md` records that no test, build, or lint config consults, and the anti-self-exemption rule held as designed: the diff adding this row was itself non-invariant (the config is on the `absent` list), and its stage 5 had in fact already run — `:full`, 98.65, on the same tree.
...
**`role-cycles.config.json`** — the per-pipeline canonical role-cycle declarations that `agent-doc-check`'s check 4 reads, added by `declare-the-role-cycles-in-config` on 2026-09-17. Never mutated (`mutate` is `src/**`); consulted by no `src/`-side config, verified by the same grep recipe as the `CLAUDE.md` entry; read in production by `scripts/agent-doc-check/run.ts` alone, whose tests build temp-repo fixtures per case and never read the real root config — a binding constraint on that program's tests, stated in the slice's design. The entry's framing is the user's 2026-09-17 config-invariance ruling: a change to a real-world config a checker script reads is mutation-invariant, because the mutation gate on the checker scripts themselves is what makes the implementation behave correctly across runtime inputs. Argued, like every entry here, for this predicate's own `src/`-run scope.

**`.vale.ini`** — added by `rationale-sidecar-pilot`. At the entry's writing, nothing in `package.json`, `vite.config.ts` or the `vitest.*.config.ts` files referenced Vale, and no `npm run` script invoked it. The script side has since rotted exactly as the next sentence predicted — `prose-lint` landed 2026-09-10 and `vale-fixture-check` after it, both spawning the binary — and the argument absorbs it: Vale could gain a script, or a gate, and the score still could not move, because Stryker runs the vitest suite and nothing else. Premise dated 2026-09-15 after `hardener` reported the rot; the conclusion never rested on it.
