...
- Run `npm run ast-grep` and read its output — a warning-severity finding does not move the exit code, so a zero exit is not evidence of anything. (A _nonzero_ exit does mean something: 8 for a rule that failed to parse, 1 for an `error`-severity match.) Whenever you added or changed a rule, also run `npm run ast-grep:test`, which is a genuine gate: a malformed rule fails it.
