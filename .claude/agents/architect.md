...
- Run `npm run ast-grep` and read its output — it always exits 0, so a silent run is not evidence of anything. Whenever you added or changed a rule, also run `npm run ast-grep:test`, which is a genuine gate: a malformed rule fails it.
