...
- **Whenever you added or changed anything under `vale-styles/`, run `npm run vale-fixture-check`.** It gates, on five binary facts about the styles you own. It fails when `vale` is absent rather than passing.

  **It cannot tell you whether a `.good` fixture discriminates, and that judgement is yours.** A good fixture proves nothing unless it carries a near-miss the rule declines for the right reason. Check it by hand at REVIEW.
