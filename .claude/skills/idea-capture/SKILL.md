---
name: idea-capture
description: File a raw thought as a candidate on the backlog/ideas/ board lane, in the template's shape.
...
- Do not add a `status:` field. The directory is the status. Do not add a `kind:` field either — `/idea-assess` writes that at assessment, not capture.
