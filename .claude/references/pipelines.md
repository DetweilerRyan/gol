...
4. `product` (SPECIFY) runs `npm run acceptance-mutation -- --feature <name>` and refines.
   Step 4 exists only on the path where step 3 happened — with no implementation every mutant
   "kills" and the run measures nothing. A contract-review-only spike goes 1 → 2 → 6.
5. The seat discards the spike implementation.
6. `product` (SPECIFY) presents the refined contract and stops for user sign-off.

`product.md` carries the conduct that binds steps 1–6: refinement may only strengthen the
contract, and approval happens once, at step 6.
...
| 4    | `cleaner` — after **every** coder invocation, never once at the end                                                                 | Coder's manifest, carried forward verbatim; the slice name                                                                               | Reads the manifest's files                                          | Its own — see `cleaner.md`               | What changed or that nothing did; survivor demonstrations                            |
| 5    | `architect` REVIEW                                                                                                                  | The mode by name — **an omitted mode silently becomes REVIEW, so name it even here**; the slice name; `design.md` when a design pass ran | Reads the landed tree against `design.md`                           | Its own — see `architect.md`             | Ruling; staleness report for the seat                                                |
| 6    | `hardener`                                                                                                                          | The manifest; whether an acceptance spike ran; any exemption instruction with its computed diff; slice, not integration                  | Reads the tree                                                      | Its own eight stages — see `hardener.md` | Per-stage results; any skip named with its instruction                               |
| 7    | `product` VERIFY                                                                                                                    | The mode by name; read the committed artifacts, not memory                                                                               | Writes `*.e2e.spec.ts`; runs the full `npm run acceptance-mutation` | Its own — see `product.md`               | Done, or one batched defect report routed to `architect` ADJUDICATE                  |
...
- **Surfaces no role may edit fall to the seat.** CLAUDE.md, articles, sidecars, role files, and
  reference files are the seat's to write, on the user's standing approval for the ruled design.
  An enabler that is mostly documentation is therefore mostly seat-executed, with `coder` and
  `cleaner` taking only the `src/` and `scripts/` share.
...
**Exit:** as the story, minus step 8's figure. Say in the merge record that the step is moot
and why — a step with no output reads as a step nobody ran.
...
- **`architect` requires a mode, and omitting it does not hard-fail.** Absent a name it reviews.
  A DESIGN, CONTRACT or ADJUDICATE request sent without its mode comes back as a plausible
  REVIEW. This is the most dangerous contract in the set, because the failure is silent.
- **The mutation-invariant exemption exists only as a thing the seat says.** `hardener` runs
  stage 5 absent an instruction, without exception. Compute the predicate with
  `npm run mutation-invariance -- --diff <range>`, name the diff, and put both in the prompt.
  Exit 0 is invariant, 2 is not, and **1 means no verdict was computed** — never read 1 as a
  pass. CLAUDE.md's merge protocol is the source of truth and wins any disagreement.
...
- **Whether an acceptance spike ran.** `hardener` checks spike residue only if told a spike
  happened, and only the seat knows. The seat also discards the spike implementation.
...
- **Re-invoke `hardener` whenever an adjudicated fix touches `src/`**, before `product`
  re-verifies. Mostly cheap — stage 5 is incremental, so cost tracks the diff.
- **Do not widen `coder`'s scope.** Property tests are `architect`'s; every quality gate belongs
  to `cleaner` and `hardener`. A design step reading "module + unit **and property** suite" is a
  split into two invocations, not one.
- **The design pass is the seat's call, not `product`'s.** Its triggers are facts about the
  shape of `src/`, which `product` is deliberately blind to. They live in CLAUDE.md's "The
  optional architect design pass".
...
- **`product`, triage bucket D** — a finding outside the slice's changed-files manifest.
  Reports and stops.
