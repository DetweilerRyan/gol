...
- **`.claude/agents/articles/prose-linting.md`** — running Vale over the agent docs, `CLAUDE.md`, the module sidecars, and the JSDoc blocks in `src/` and `scripts/`. It says which of the six enabled `STE` rules apply mechanically and which are prompts to look, and it lists the exempt classes for each. It also carries the ways a run reports a confident zero, and how a pass damages the file it cleans. `architect` alone authors the tracked `vale-styles/JsDoc` style, on the `rules/*.yml` precedent, and the article carries its fixture run. Read before acting on a Vale finding, and before enabling or re-levelling a rule.
...
**Nine programs, and only the four below gate.** Four of the other five are advisory — `scripts/acceptance-mutation/` (owned by `product`, not `hardener`), `scripts/gherkin-dry-checker/`, `scripts/halstead4ts/`, and `scripts/perf-report/` — are described in **`.claude/agents/articles/quality-tooling.md`**, and the acceptance-mutation runner has its own article, **`.claude/agents/articles/acceptance-mutation.md`**. The fifth is `scripts/prose-lint/` (`npm run prose-lint`), which runs Vale over the tracked file list. It never fails on a finding, and fails when it cannot lint. **It is also the only program here that is not TypeScript, which `architect` ruled a defect on 2026-09-10** — a `.sh` reaches none of `test:scripts`, `crap4ts:scripts`, `dry4ts:scripts`, `test:mutation:scripts`, or `reference-check`'s comment scan. See `ideas/candidates/prose-lint-runner-is-shell-not-typescript.md`. The four gating checkers keep their full entries here, because they are what polices `rules/`, `.claude/**`, the comment/doc-comment surface, and the mutation-invariance allowlist itself:
