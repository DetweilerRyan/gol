...
**Eight programs, and only the four below gate.** The four advisory ones — `scripts/acceptance-mutation/` (owned by `product`, not `hardener`), `scripts/gherkin-dry-checker/`, `scripts/halstead4ts/`, and `scripts/perf-report/` — are described in **`.claude/agents/articles/quality-tooling.md`**, and the acceptance-mutation runner has its own article, **`.claude/agents/articles/acceptance-mutation.md`**. The four gating checkers keep their full entries here, because they are what polices `rules/`, `.claude/**`, the comment/doc-comment surface, and the mutation-invariance allowlist itself:
