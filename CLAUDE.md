...
- **`.claude/agents/articles/prose-linting.md`** — running Vale over the agent docs. It says which of the six enabled rules apply mechanically and which are prompts to look, and it lists the exempt classes for each. It also carries the five ways a run reports a confident zero, and how a pass damages the file it cleans. Read before acting on a Vale finding, and before enabling or re-levelling a rule.
...
4. Is it **depth about one module** that overflows a JSDoc hover? → a **pair of sidecars beside the source**, split by filename so the register is visible before a word is read. `<module>.md` holds extended examples, use cases and best-practice notes, for whoever is _calling_ it. `<module>.rationale.md` holds measurements, rejected alternatives and corrections, for whoever is _changing_ it. Either may be absent, and usually one is. Reference from the JSDoc as `@see {@link ./useZoomGlide.rationale.md}`.
...
5. Is it **the evidence behind a rule in an article**, rather than the rule itself? → a sidecar `<article>.rationale.md` **beside the article** (`doc-comments.rationale.md` next to `doc-comments.md`). Evidence means a measurement, a probe method, a rejected alternative, or a correction. No role carries a read trigger for one, and that is the point. It is read when a rule is being changed, never in order to follow it.
   ...
   **Evidence about a single module goes beside that module, under branch 4, even when it is also evidence behind an article's rule.** The rule stays in the article and the worked case moves to `<module>.rationale.md`. `src/scrollbars.rationale.md` under `architecture.md`'s provenance rule is the live precedent.
...
Its layout is two levels and no more. **`scripts/<program>/` holds one program's own modules** — its `run.ts` shell plus the pure modules that shell delegates to. **A file at `scripts/` root is shared by two or more programs**, and `scripts/feature-files.ts` is the current example. **Imports run program → shared and never the reverse.**
...
- `scripts/ast-grep-rule-check/` (`npm run ast-grep:rules`) — **this is the first of three checks in this list that gate.** Every other check in this list needs judgment to act on. This one is a set of binary facts about `rules/*.yml` and `rule-tests/*.yml`:
...
- `scripts/agent-doc-check/` (`npm run agent-doc-check`) — **the second of three checkers in this list that gate**, over `.claude/**` + CLAUDE.md itself rather than over `rules/`. Five binary facts:
  ...
  2. Every `.claude/agents/*.md` file's frontmatter validates — `name` matches the filename, `description` is non-empty, `tools` is a subset of the known tool set, and `model` is one of `opus`/`sonnet`/`haiku`. It is parsed with a bespoke line-anchored reader rather than a real YAML parser. Several agent descriptions contain a literal `": "` that a real YAML parser reads as a nested mapping and refuses. Measured to fail exactly this way on 2 of the 5 agent files as they stand.
  3. No backticked mention of a retired role (`qa`, `refactorer`, `specifier`) without a historical qualifier (old/former/then/merge/...) on the same line. That is a short, git-verified list of every role ever deleted from `.claude/agents/`, not a generic "role-shaped token" scan. The generic form was tried and rejected, for flagging six unrelated backticked identifiers to two real hits against this repo's own docs.
...
All seven run via `tsx`, each as `tsx scripts/.../run.ts`: `npm run halstead4ts`, `acceptance-mutation`, `gherkin-dry`, `perf-report`, `ast-grep:rules`, `agent-doc-check`, `reference-check`. Each has its own `.test.ts` unit tests, runnable via `npm run test:scripts`, which is a dedicated Node-environment vitest config. `vite.config.ts` excludes `scripts/**` from the main `npm test` and `npm run test:unit` run, so scripts' tests no longer run there.

**The ast-grep structural rules.** `sgconfig.yml` + `rules/*.yml` are an eighth checker, and the only one that is not a `scripts/` program. [ast-grep](https://ast-grep.github.io) structural rules encode the architectural invariants that were previously prose only.
...
`no-dead-doc-on-annotated-return-literal` joins that list from the other direction. It fires on documentation rather than on code, the moment a new hook with an annotated return type carries a JSDoc block on a property of its `return { … }` literal. The measured evidence says that placement reaches no caller.
...
`vale sync` is **not** optional and `npm ci` does not cover it. `.vale/` holds the downloaded style package and is gitignored, so a fresh worktree has none, and a run without it reports zero findings through a `grep -c` pipeline rather than an error. `.claude/agents/articles/prose-linting.md` carries that mechanism, and the other ways a run reports a confident zero.
...
5. **Run the full gate on `main`.** Mandatory, even though step 3 just passed on an identical tree — step 3 ran against a different Stryker cache and a different `coverage/`. This run is what rebuilds `main`'s own caches into a state the next merge can trust — **unless this is a mutation-invariant merge, in which case skip this deletion entirely; read the clause below before running it**:
   ...
   then invoke `hardener` on `main` with the whole tree as its scope. Deleting the cache is the honest expression of intent: it is not stale, it is describing a tree that no longer exists — the situation `.claude/agents/articles/mutation-testing.md` already names.
