...
5. **Run the full gate on `main`.** Mandatory, even though step 3 just passed on an identical tree — step 3 ran against a different Stryker cache and a different `coverage/`. This run is what rebuilds `main`'s own caches into a state the next merge can trust. **The one exception: if this is a mutation-invariant merge, skip the deletion below entirely.** Read the clause below before running it:
   ...
   **`.oxlintrc.json` was added by the same slice, and it needs no precondition.** It is safe as a matter of fact rather than of configuration. That is the `CLAUDE.md` and `README.md` category: a fixed filename cannot match a test glob, so no future diff turns this path into a test. It is not mutated, `mutate` being `src/**`. It _is_ tracked, and therefore copied into the sandbox, but nothing there consults it. Stryker runs the vitest suite and nothing else, and oxlint is referenced by neither `vite.config.ts`, `vitest.scripts.config.ts`, `src/test-setup.ts`, `fast-check-stryker-seed.ts` nor either Stryker config. Verified 2026-09-08 by grepping those six files.
   ...
   Note what the entry does **not** license. `.oxlintrc.json` gates `npm run lint`, which is not one of `hardener`'s eight stages at all. It is the per-role step in `.claude/agents/articles/workflow.md`. So a change here can still red something, just never stage 5.
   ...
   **Each of the three points is guarded by something different, and only two of the three are guarded at all.** Point 1 holds because `stryker.config.json` is itself on the absent list, so widening `mutate` re-arms the gate in the same diff. Point 2 is cured by configuration, as the next paragraph describes. **Point 3 is guarded by nothing, and it is the one an ordinary slice breaks.** A `scripts/` test asserting an invariant over the live `rules/` directory would land through a normally-gated `scripts/` diff, one that mentions this allowlist nowhere. That is exactly the shape `scripts/acceptance-mutation/examples-cell-sites.test.ts` already has for `features/`. Every `rules/`-only diff would be wrongly exempt from that moment on. Re-derive point 3 rather than reading it forward.
   ...
   **`.gitignore` is deliberately _not_ added, and the asymmetry is the point rather than an inconsistency.** The two files landed in the same slice, so the omission is a ruling. Stryker's sandbox is populated from tracked files. The paragraph below the `rm -f` already rests on this, in observing that `ideas/`, `.claude/` and `CLAUDE.md` are tracked and therefore _are_ copied. So a `.gitignore` edit can change what the sandbox contains. That is a path to moving the score, even though the file itself yields no mutant.

   `.prettierignore` is left out for the weaker reason that an allowlist should carry only what has been argued, and nobody has needed it. **Adding a path here requires the argument, not the intuition.** The failure mode of a wrong entry is a skipped stage that reads exactly like a passing one.
