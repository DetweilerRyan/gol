...
- **`.claude/references/definition-of-ready.md`** — the idea board's readiness assessment: the kind discriminator, the six anchored predicates, the four dispositions, and the calibration record. It also states the assessment record's shape. Its sidecar carries the provenance, the vocabulary-collision census, and the rejected alternatives. The audience is the orchestrating seat and the `idea-*` skills — no role assesses a candidate. Read before assessing a candidate, promoting an idea, or ruling a disposition.
...
- **Articles that do not** — `architecture`, `archive`, `claim-discipline`, `handoffs`, `orchestration`, `workflow`. `claim-discipline` has its evidence elsewhere rather than nowhere: it shares `engineering.meta.md`, as the house-rules pointer above says. The other five have none.
