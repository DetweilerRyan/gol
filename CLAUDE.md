...
Eleven are topic articles, read on the trigger each one names in its own header:
...
- **`.claude/agents/articles/backlog-readiness.md`** — the idea board's readiness assessment: the kind discriminator, the six anchored predicates, the four dispositions, and the calibration record. Its sidecar carries the provenance, the vocabulary-collision census, and the rejected alternatives. The audience is the orchestrating seat — no role reads the board. Read before assessing a candidate, promoting an idea, or ruling a disposition.
...
- **Articles that have one** — `acceptance-mutation`, `ast-grep-rules`, `backlog-readiness`, `doc-comments`, `engineering`, `mutation-testing`, `prose`, `quality-tooling`, `state-flow`, `testing-layers`.
...
- **`ideas/todo/`** — examined, and concrete enough to hand to `product`. The orchestrating session owns this board: no role reads it, and an idea reaches `product` as prompt content rather than as a file path. Keep this to about three, matching the two-or-three concurrent-slice ceiling below. A `todo/` lane longer than the number of slices you can actually run is a candidates lane wearing a different hat. Promotion is assessed against `.claude/agents/articles/backlog-readiness.md` — advisory, since this board has no gate.
