...
4. Is it **depth about one module** that overflows a JSDoc hover? → a **pair of sidecars beside the source**, split by filename so the register is visible before a word is read. `<module>.md` is for whoever is _calling_ the module, and `<module>.rationale.md` for whoever is _changing_ it. Which half a given sentence belongs in is a question for `prose-linting.md`, not for this branch. Either may be absent, and usually one is. Reference from the JSDoc as `@see {@link ./useZoomGlide.rationale.md}`.
   ...
   **Note what `npm run reference-check` actually reaches.** A **repo-relative** token naming a sidecar is resolved, since `check-md-references` added `.md` to the extractor, and a **leading-dot** token like the mandated `@see {@link ./useZoomGlide.rationale.md}` form now resolves too, by its own basename, since `reference-check-reach` closed that gap. What the checker verifies is that the _name_ resolves somewhere in the tree, not that the _relative path_ in front of it is right -- `{@link ./<name>.md}` written in `src/hooks/` resolves against a `<name>.md` anywhere in the repo, the same deliberate basename-matching under-report every other check in this program makes. Nothing checks a sidecar's **contents** either, and a sidecar moved to another directory still resolves under its old name.
...
- `scripts/reference-check/` (`npm run reference-check`) — **the third of four checkers in this list that gate**, over the comment surface itself rather than over `rules/` or `.claude/**` alone. The defect it exists to catch: a comment or a doc line names another file, or a symbol inside one. That target is then renamed or deleted, and nothing notices.
  ...
  It scans comment lines only — trimmed start `//`, `*`, `/*`, or `#` — in every `.ts`/`.tsx`/`.yml`/`.yaml` file under `src/`, `scripts/`, `features/`, `perf/`, `rules/`, `rule-tests/`, `vale-styles/`, and at the repo root, minus `src/catalyst/` (the vendored library boundary). It also scans **every** line of every tracked/untracked-not-ignored `.md` file in the repo, minus `ideas/**` (an idea file's own worked examples are dead references by design) and `.claude/worktrees/` (the same sanctioned exclusion `agent-doc-check` carries) — not an enumerated `CLAUDE.md`/`README.md`/`.claude/**/*.md` list, which is what left `vale-styles/JsDoc/README.md` and 15 other citations in that directory unchecked until `reference-check-reach`. A doc file's prose is itself the thing being checked, rather than commentary around code.
