...
**Six programs, and only the two below gate.** The four advisory ones — `scripts/acceptance-mutation/` (owned by `product`, not `hardener`), `scripts/gherkin-dry-checker/`, `scripts/halstead4ts/`, and `scripts/perf-report/` — are described in **`.claude/agents/articles/quality-tooling.md`**, and the acceptance-mutation runner has its own article, **`.claude/agents/articles/acceptance-mutation.md`**. The two gating checkers keep their full entries here, because they are what polices `rules/` and `.claude/**` themselves:
...
- **Accessible names are sentence case** — capital on the first word only, and no role word inside the name (AT appends "group", "button" itself). Every name this app ships follows it: `Cell 3, 5`, `Zoom in`, `Zoom out`, `Reset view`, `Open pattern library`, `Pattern preview cell 3, 5`, `Horizontal scroll`, `Vertical scroll`, `Column ruler`, `Row ruler`.
- **One name is deliberately title case: the pattern library dialog is `Pattern Library`.** It names a **place in the app** rather than describing an action or a thing, which is the same reason a document's title keeps its own capitalisation — and the button that opens it stays sentence case (`Open pattern library`), because that one _is_ an action. The exception is stated here because this line previously claimed "every name this app ships follows it" while listing `Pattern library`, a name the app has never shipped: `src/components/PatternLibraryModal.tsx` renders `Pattern Library`, and `PatternLibraryModal.test.tsx`, `LifeBoard.test.tsx` and `features/screenplay/elements.ts` all assert that form. The convention was the only thing out of step, and `elements.ts` had already recorded the lowercase form as "wrong case" beside the locator that once matched it. **The scope of the exception is one dialog title, not a licence for title case generally** — and note `elements.ts` matches it with `exact: true`, because `getByRole`'s default matching is case-insensitive _and_ substring, so a future drift back to `Pattern library` would pass a lax matcher silently.
