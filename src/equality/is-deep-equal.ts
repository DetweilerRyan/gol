 ...
 * worse than isShallowEqual's rather than equal to it. Never reach for
...
export function isDeepEqual<T>(a: T, b: T): boolean {
  ...
  //
  // Every level of nesting re-runs the whole container walk, and a Set of
  // non-primitives degrades to an O(n*m) scan at each level --
  // `Set.has`/`delete` can only match members by SameValueZero, so
  // structurally-equal-but-distinct references (the case this comparator
  // exists for) always fall through to a linear scan over the remaining
  // members, each step of which is itself a recursive isDeepEqual.
