...
export interface Anchor {
  /** World coordinates, tile-aligned to whatever `spanCells` produced this anchor -- see {@link computeAnchor}. */
  x: number
  /** World coordinates, tile-aligned to whatever `spanCells` produced this anchor -- see {@link computeAnchor}. */
  y: number
}
...
/**
 * A fresh anchor near the camera's current offset, tile-aligned to
 * `spanCells` so it lines up with the tile grid cellTiles.ts mounts against.
 *
 * @param spanCells Must match the span cellTiles.ts tiles with, or the two
 * grids drift apart.
 */
// Floor-toward-negative-infinity, the same convention as cellTiles.ts's
// tileIndexOf / tileOriginCell pair, duplicated here rather than imported
// (see this module's header on why the two stay independent).
 ...
 * {@link ANCHOR_DRIFT_CELLS} on both axes -- the precision-bounding half of
 * the no-infinite-loop guarantee {@link nextAnchor} depends on.
 */
...
/**
 * The sticky-anchor rule: keeps the existing anchor while it still holds
 * (per {@link anchorHolds}), re-quantising onto a fresh one otherwise.
 *
 * Applying this to its own result is a no-op -- re-running it against the
 * anchor it just returned yields that same anchor again, which is what makes
 * repeated calls per render safe.
 *
 * @returns `previous`, by reference, when it still holds -- a caller gating
 * a re-render on `current !== anchor` depends on that identity, not just on
 * structural equality.
 */
 ...
 * camera -- apply this as the translate on a layer wrapping every mounted
 * cell, so a pan moves that one offset instead of every cell's own position.
 */
// This is latticeOffsetPx, renamed: deliberately just worldToScreen --
// screenToWorld (used to resolve taps and hover back to a world cell) must
// agree with wherever the anchor visually painted, and worldToScreen is the
// one function both this and screenToWorld are already inverses of --
 ...
 * anchor -- independent of `camera.offsetX`/`offsetY`, which is what lets a
 * pan avoid re-rendering a retained cell's position at all.
 *
 * Composes with {@link anchorOffsetPx}: `anchorOffsetPx(anchor, camera)` +
 * `cellOffsetPx(worldCoordinate, anchorCoordinate, cellSize)` equals
 * `worldToScreen(camera, worldCoordinate)` exactly.
 */
// Scalar in, scalar out -- kept separate from returning an {x, y} pair so
// the hot per-cell render loop stays allocation-free and callers pass React
// Compiler primitive props rather than a fresh object every render. This is
// the same identity slotPixelPosition + latticeOffsetPx used to guarantee,
// now split across anchor and per-cell offset instead of lattice origin and
// slot index (see cellAnchor.property.test.ts).
