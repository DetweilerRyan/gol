...
- Does the skill system resolve a script path relative to the skill, or only through the two
  encoded references? If only the latter, relocation is a two-file edit rather than a
  contract change.
- Does `on-edit.ts` share `layer1.ts`'s bare-node constraint, or was that constraint written
  for one program?
- Does a scoped vitest project pull in anything else under `.claude/**` that should not be
  collected? The write-time hooks are the only two programs there today.
- Which of the two programs is the harder case, and does one answer cover both?
