...
| 2   | **Landed** — `slice/extract-the-merge-protocol`, 2026-09-17; folder awaiting retrospective in `done/`     | Item 4 — the merge protocol out of CLAUDE.md into `.claude/references/`, every in-boundary source-of-truth citation repointed in the same slice, routing branch 1's rationale restated. The three out-of-boundary citations are their own capture.       |
...
| 4   | `backlog/ideas/epics-promote-to-their-own-lane.md` and `backlog/ideas/epic-pipeline-runs-the-children.md` | Item 7 — split out at collection time as one candidate, then split again on 2026-09-21: the `backlog/epics/` lane and its child roster in the first, the Epic pipeline in the second. Both supersede the no-folder ruling.                               |
...
- Child 1 leaves one forward dependency on child 4: `pipelines.md`'s Epic section keeps its
  "no pipeline" sentence until `epic-pipeline-runs-the-children` lands, whichever order they
  run in. The 2026-09-21 split moved that dependency off the lane candidate, which no longer
  touches the Epic section at all.
