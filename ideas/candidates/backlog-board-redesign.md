...
adr/                         # PARTLY LANDED -- see below
  README.md
  TEMPLATE.md
  0001-….md                  # subject re-scoped at reevaluation point 7 -- see the adr/0001 row
...
### The pipeline reference — ruled 2026-09-16 at reevaluation, point 8

The problem it solves: no surface states a slice's pipeline end to end.
Answering "what happens, in order, for an enabler?" today means synthesizing
CLAUDE.md's cycle string and design-pass triggers, `orchestration.md`'s
invocation contracts and sequencing rules, `handoffs.md`'s handoff shape, the
role files' own handoff closings, and this file's kind→cycle mapping. The
per-kind variations exist only as scattered exceptions.

**The deliverable: `.claude/references/pipelines.md`**, with a
`pipelines.meta.md` sidecar for the rationale, per the standing pair
convention. One section per kind — story, enabler, spike, epic — each stating
the complete class of service (the Kanban term for exactly this: explicit
policies, per work item type, for how an item is treated through the system):

- **Entry** — what promotion creates for this kind (folder shape, `kind:`).
- **A Mermaid flow diagram plus a step table** (user-ruled: both). Table
  columns: role + mode; what the invoking prompt must carry (the invocation
  contracts for that step); artifacts read and written (`proposal.md`,
  `design.md`, `tasks.md`, `findings.md`, `features/`); gates the step runs;
  the handoff it closes with, citing `handoffs.md` for the shape. The two
  representations are kept consistent by hand — nothing checks a diagram.
- **Exit** — the `done/` move, the tag, a merge-protocol pointer, the
  retrospective.
- One cross-kind section: the escalation lanes as pipeline interrupts.

**Single-sourcing is by moving, not copying** (user-ruled): `orchestration.md`'s
"invocation contracts" and "sequencing this seat owns" sections migrate in,
leaving that article seat conduct plus a pointer; CLAUDE.md's "Subagent
pipeline" section slims to the routing summary it already claims to be,
keeping the cycle string; `handoffs.md` keeps the generic handoff shape; role
files are untouched — a role stays kind-blind and mode-told.

**Why `references/`**: the tier's stated purpose is files that skills, roles,
and the seat may all name as a read trigger, and the consumers are exactly
that set — the seat composes every invocation from it, `/idea-assess` assigns
the kind that selects a pipeline, `/idea-promote` creates the folder shape a
pipeline consumes, and `definition-of-ready.md` sits beside it owning the kind
check. The standing gates reach the new file for free: `agent-doc-check`'s
...
| Pipeline reference | **Adopted 2026-09-16 at reevaluation point 8** — see "The pipeline reference" section: `.claude/references/pipelines.md` states each kind's full class of service (entry, Mermaid flow plus step table, exit, escalation interrupts), single-sourced by migrating `orchestration.md`'s invocation-contracts and sequencing sections in and slimming CLAUDE.md's pipeline section to routing. Part of this item, which the user ruled becomes an epic split into child candidates next.                                                                                                                                                                                                                                                 |
...
redesign. All reevaluation points are now ruled; the steps and Touches get
redrawn at the epic split the user ruled after point 8 — this file becomes an
index over child candidates, per `definition-of-ready.md`'s epic disposition.
