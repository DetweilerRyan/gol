...
| `split-state-flow-article`          | `state-flow.md`          | landed |
| `split-acceptance-mutation-article` | `acceptance-mutation.md` | landed |

**Seven splits done. One target left of the eight in the Complication's table: `architecture.md`.**
Measured 2026-09-10 against `.claude/agents/articles/`: every other row of that table now has a sidecar,
and `archive.md` was ruled to keep its name rather than take one. `architecture.md` carries **no heading
structure** — one H3 over a flat module list — so routing prose into it means inventing headings it does
not have.

**The rollout grew a second half this candidate never scoped: the role files.** `split-architect-role`,
`split-hardener-role` and `split-cleaner-role` gave three role files a sidecar; `split-product-role`,
`split-coder-role` and `split-orchestration-article` ruled that the other three did not need one. The
creation threshold now stated in CLAUDE.md's routing branch 5 came out of those six rulings.
