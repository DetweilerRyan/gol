...
| `mechanise-prose-soundness-with-a-style-package`     | **Reoriented 2026-09-12: audit the rules we have before adopting more.** Neither overlapping `Std` rule moves, each rejected for its own measured reason. The deeper finding: **Vale cannot express "a long sentence that also contains a clause boundary"** — `occurrence` counts and cannot match, `existence` matches and cannot count, `sequence`'s `skip` bounds a gap only from above. That is why every published rule is a single predicate, and why a defect that is a conjunction of two conditions is invisible to all of them. A `scripts/` checker is the shape that fits.                                           |
