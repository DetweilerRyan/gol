...
- **Nothing in the pipeline runs the linter.** `npm run prose-lint` exists, no role invokes it, and it
  ...
  made it answerable: `scope-prose-lint-then-give-roles-the-trigger` gives the four implementing roles a trigger, once the runner can be scoped to a slice.
