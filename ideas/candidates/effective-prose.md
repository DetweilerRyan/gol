...
| `split-engineering-by-audience`                      | **Filed 2026-09-11 from the role-file strips.** Every role is told to read `engineering.md` unconditionally; it is 43,742 bytes across fourteen sections, and four of them plausibly bind `product`. The blanket read line in all five role files is the mechanism. Claim discipline is the clearest mis-sort — it binds the orchestrating session too and is not an engineering rule. **Sequence after the five strips**, which are what reveal each role's real audience.                                                                                                                                                       |
