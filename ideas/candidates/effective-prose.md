...
| `mechanise-prose-soundness-with-a-style-package`     | **Reoriented 2026-09-12: audit the rules we have before adopting more.** The ten enabled rules report 1,654 findings over 435 files, and two `STE` rules needing judgement are 88 percent of their style's total. Two findings settle the `Std` question per rule rather than per package: `PassiveVoice` disagrees both ways because `Std` matches any `-ed` word after a be-verb where `STE` asks a POS tagger, and `SentenceLength`'s 149 extra findings are all 26-29 words, caused by a tokenizer that splits `call-site` into two. **Neither moves.** What remains is per-surface precision, since `HistoricalNarration` scored 8/8 where written and 1-in-3 on `src/`. |
