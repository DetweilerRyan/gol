...
| `reference-check-reach`                               | **Bundles three changes to one program**, two of which edit the same predicate and so cannot run concurrently. 15 unchecked citations in the linter's own directory; the leading-dot discard that hides the mandated `@see {@link ./x.rationale.md}` form; and the same discard hiding every dot directory, `.claude/**` included. The two folded entries disagree on what the predicate should become, and settling that is the slice's central question. |
...
Land order is C then A then B: the vitest exclusion first because it is one line, then the runner
