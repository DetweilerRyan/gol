...
| `spike-vale-rules-that-compress-the-agent-files`    | **A spike, and its output is rules rather than a rewrite.** The five role files hold 15,007 words; the enabled six find 195 on them — **all of it two rules**, `ProcedureLength` 122 and `PassiveVoice` 73, the other four already at zero — and the full `STE` + `Slop` set finds 582, and neither `Microsoft` nor `Google` is installed. Applies rules one at a time, measuring whether each drives a rewrite that is shorter and says the same thing. **Spikes `coder.md` first and `architect.md` last** — and `coder.md` is measurably the denser target at 45.2 findings per 1000 words against `architect.md`'s 39.9, which is evidence that the big file's length is a routing problem rather than a prose one. **Subsumes the `Microsoft`/`Google` half of the row below** — reconcile the two rather than running both. |
