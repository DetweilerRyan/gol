...
| `mechanise-prose-soundness-with-a-style-package`     | **Reoriented 2026-09-12 on the user's direction: audit the rules we have before adopting more.** Measured over 435 files, the ten enabled rules report 1,654 findings, and two `STE` rules needing judgement are 88 percent of their style's total. The sharper finding is that a rule's precision does not travel: `HistoricalNarration` scored 8/8 on the file it was written against and 1-in-3 on a sampled `src/` surface it was never tuned on. Re-measure per surface first; the `Std`/`Google`/`Microsoft` survey follows.                                                                                                |
